...
if ($id == 'filtersettingsiyavula') { // Only if the current page is the filter siyavula settings
    ...
    if (!empty($getusers) && !empty($tokenresponse)) {
        foreach ($getusers as $user) {
                $data[] = $user->email;
                $testtoken = '<a target="_blank" href="' . $CFG->wwwroot . '/filter/siyavula/test_external_usertoken.php?token=' . $tokenresponse . '">' . get_string('test_external_usertoken', 'filter_siyavula') . '</a>';
        }
